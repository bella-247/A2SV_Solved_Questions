...
Difficulty: 1400
Time: 45 min
URL: https://codeforces.com/contest/1374/submission/362400114
...
 
 
def calculate(count, k, remain):
    if remain == 0:
        return 0
    
    return (count - 1) * k + remain + 1
 
for _ in range(int(input())):
    ...
    counts = Counter()
    for num in nums:
        remain = remaining(num, k)
        counts[remain] += 1 
 
    maxx = max(counts, key= lambda x : calculate(counts[x], k, x))
    
    print(calculate(counts[maxx], k, maxx))