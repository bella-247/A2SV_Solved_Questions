...
Difficulty: 1100
Time: 45 min
URL: https://codeforces.com/contest/2070/submission/362725555
...
 
    seconds_0_to_0 = (
        0  # stores the number of steps needed to return to zero, after starting at zero
    )
    point = 0
...
 
        if point == 0:
            # add one to the index, so that it could be count as index
            seconds_0_to_0 = i + 1
            break
 
    point = x
...
 
    ...
    if zero:
        count += 1
        if seconds_0_to_0 > 0:
            seconds_left = k - seconds_x_to_0
            count += seconds_left // seconds_0_to_0
 
    print(count)