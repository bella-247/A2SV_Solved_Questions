...
class Solution:
    def createSortedArray(self, instructions: List[int]) -> int:
        MOD = (10 ** 9) + 7
        n = len(instructions)
        indexed_instructions = [[num, i] for i, num in enumerate(instructions)]

        smaller_greaters = [[0, 0] for _ in range(n)]
        ...
        def mergeSort(left, right):
            if left == right:
                return [indexed_instructions[left]]
        ...
        return sum(min(smaller, greater) for smaller, greater in smaller_greaters) % MOD
